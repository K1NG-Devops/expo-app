...
export default function ParentJoinByCodeScreen() {
  ...
  const onJoin = async () => {
    ...
    try {
      ...
      if (error) throw error;
      Alert.alert('Joined!', 'Your account is now linked to the school.');
      ...
      {validated && (
        <View style={styles.card}>
          ...
          <TouchableOpacity style={[styles.button, styles.joinBtn]} onPress={onJoin} disabled={redeeming}>
            {redeeming ? <ActivityIndicator color={theme?.onPrimary || '#000'} /> : <Text style={styles.buttonText}>Join School</Text>}
          </TouchableOpacity>
        </View>
      )}
    </View>
