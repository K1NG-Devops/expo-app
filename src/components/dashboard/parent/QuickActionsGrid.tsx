  ...
  const getQuickActions = (): QuickAction[] => {
    ...
    if (!hasOrganization) {
      return [
        ...
        { icon: Lightbulb, label: 'ECD Play', href: '/ecd/play', color: '#22c55e' },
        { icon: Target, label: 'Enter Class Code', href: '/dashboard/parent/class-code', color: '#0ea5e9' },
      ];
    }
    ...
    <div className="section">
      ...
      >
        {actions.map((action) => {
          ...
          const isDashChat = action.label === 'Chat with Dash';
          
          return (
            <button
              key={action.href}
              onClick={() => router.push(action.href)}
              className={`qa ${isDashChat ? 'dash-chat-action' : ''}`}
              style={{
            ...
            >
              {isDashChat && (
                <div style={{
                  ...
                  animation: 'dashInnerPulse 2s ease-in-out infinite',
                  pointerEvents: 'none'
                }} />
              )}
              <div style={{
      ...
      <style jsx>{`
        .dash-chat-action {
          ...
          background: linear-gradient(135deg, rgba(236, 72, 153, 0.05), rgba(236, 72, 153, 0.1)) !important;
        }

        @keyframes dashInnerPulse {
          0%, 100% {
            opacity: 0.3;
