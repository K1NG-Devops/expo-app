...
import { useTeacherExamFeed } from '@/lib/hooks/teacher/useTeacherExamFeed';
...
export default function TeacherDashboard() {
  ...
  const [lang, setLang] = useState<string>('en-ZA');
  ...
  const isStandalone = !preschoolId; // standalone teacher has no org/preschool
  ...
  useEffect(() => {
    ...
    initAuth();
    // Load preferred language for AI/TTS
    try {
      const stored = typeof window !== 'undefined' ? localStorage.getItem('dash_lang') : null;
      const map: Record<string, string> = { en: 'en-ZA', af: 'af-ZA', zu: 'zu-ZA', xh: 'xh-ZA', nso: 'nso-ZA' };
      if (stored && map[stored]) setLang(map[stored]);
    } catch {}
  }, [router, supabase]);

  // Load dashboard data
  const { metrics, classes, loading: dashboardLoading, refetch } = useTeacherDashboard(userId);
  const { items: examFeed, loading: feedLoading, error: feedError } = useTeacherExamFeed(profile?.preschoolId || null);
  ...
  return (
    <>
      ...
      {/* Recent Student Exam Activity */}
      <div className="section">
        <div className="sectionTitle">Recent Student Exam Activity</div>
        <div className="card" style={{ padding: 16 }}>
          {feedLoading ? (
            <div style={{ color: 'var(--muted)' }}>Loading feed…</div>
          ) : feedError ? (
            <div style={{ color: 'var(--danger)' }}>Failed to load: {feedError}</div>
          ) : examFeed.length === 0 ? (
            <div style={{ color: 'var(--muted)' }}>No recent activity.</div>
          ) : (
            <div style={{ display: 'grid', gap: 12 }}>
              {examFeed.slice(0, 10).map((it) => (
                <div key={it.id} style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center' }}>
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <strong>{it.studentFirstName || 'Learner'} {it.studentLastName || ''}</strong>
                    <span style={{ fontSize: 12, color: 'var(--muted)' }}>{it.subject} · {it.grade}</span>
                  </div>
                  <div style={{ fontWeight: 700 }}>{Math.round(it.percentage)}%</div>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="section">
        ...
        <div className="grid2">
          ...
          <button className="qa" onClick={() => router.push('/dashboard/teacher/class-codes')}>
            <FileText className="icon20" />
            <span>Class Codes</span>
          </button>
          <button className="qa" onClick={() => router.push('/dashboard/teacher/activities')}>
            <PlusCircle className="icon20" />
      ...
      {/* Dash AI (Inline) */}
      <div className="section">
        <AskAIWidget inline language={lang} />
      </div>
       </TeacherShell>
     </>
   );
