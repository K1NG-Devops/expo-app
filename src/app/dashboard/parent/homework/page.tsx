  ...
  useEffect(() => {
    ...
    const fetchHomework = async () => {
      ...
      try {
        ...
        // Calculate stats
        const pending = data?.filter(hw => !hw.submissions || hw.submissions.length === 0).filter(hw => hw.due_date >= today).length || 0;
        const overdue = data?.filter(hw => (!hw.submissions || hw.submissions.length === 0) && hw.due_date < today).length || 0;
        const completed = data?.filter(hw => hw.submissions && hw.submissions.length > 0).length || 0;
        setStats({ pending, completed, overdue, total: data?.length || 0 });
      } catch (err: any) {
