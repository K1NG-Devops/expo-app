...
export default function SettingsPage() {
  ...
  const [showExamReminders, setShowExamReminders] = useState(true);
  const [aiVoiceEnabled, setAiVoiceEnabled] = useState(false);
  const [dataUsageMode, setDataUsageMode] = useState('standard'); // 'low', 'standard', 'high'
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      // Save to localStorage for now (could be saved to Supabase user metadata later)
      if (typeof window !== 'undefined') {
        ...
        localStorage.setItem('autoSaveHomework', autoSaveHomework.toString());
        localStorage.setItem('showExamReminders', showExamReminders.toString());
        localStorage.setItem('aiVoiceEnabled', aiVoiceEnabled.toString());
  ...
  useEffect(() => {
    const initAuth = async () => {
      ...
      if (typeof window !== 'undefined') {
        ...
        const savedAutoSave = localStorage.getItem('autoSaveHomework');
        const savedExamReminders = localStorage.getItem('showExamReminders');
        const savedAiVoice = localStorage.getItem('aiVoiceEnabled');
        const savedDataMode = localStorage.getItem('dataUsageMode');
        ...
        if (savedAutoSave) setAutoSaveHomework(savedAutoSave === 'true');
        if (savedExamReminders) setShowExamReminders(savedExamReminders === 'true');
        if (savedAiVoice) setAiVoiceEnabled(savedAiVoice === 'true');
        if (savedDataMode) setDataUsageMode(savedDataMode);
      }
      ...
      <div className="container">
        ...
        <div className="section">
          <div style={{ maxWidth: 800, display: 'grid', gap: 'var(--space-4)' }}>
              ...
              <div style={{ display: 'grid', gap: 'var(--space-3)' }}>
                ...
                <div className="listItem">
                  ...
                  >
                    <span 
                      className="toggleThumb" 
                      style={{ transform: pushNotifications ? 'translateX(20px)' : 'translateX(0)' }} 
                    />
                  </button>
                </div>
              </div>
            ...
            <div className="card">
              <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', marginBottom: 'var(--space-4)' }}>
                <Settings className="icon20" style={{ color: 'var(--primary)' }} />
                <h2 className="h2" style={{ margin: 0 }}>App Preferences</h2>
              </div>
              <div style={{ display: 'grid', gap: 'var(--space-3)' }}>
                <div className="listItem">
                  <div>
                    <div style={{ fontWeight: 600, marginBottom: 4 }}>Auto-save Homework</div>
                    <div className="muted" style={{ fontSize: 12 }}>Automatically save homework progress</div>
                  </div>
                  <button 
                    onClick={() => setAutoSaveHomework(!autoSaveHomework)}
                    className={`toggle ${autoSaveHomework ? 'toggleActive' : ''}`}
                  >
                    <span 
                      className="toggleThumb" 
                      style={{ transform: autoSaveHomework ? 'translateX(20px)' : 'translateX(0)' }} 
                    />
                  </button>
                </div>
                <div className="listItem">
                  <div>
                    <div style={{ fontWeight: 600, marginBottom: 4 }}>Exam Reminders</div>
                    <div className="muted" style={{ fontSize: 12 }}>Show notifications for upcoming exams</div>
                  </div>
                  <button 
                    onClick={() => setShowExamReminders(!showExamReminders)}
                    className={`toggle ${showExamReminders ? 'toggleActive' : ''}`}
                  >
                    <span 
                      className="toggleThumb" 
                      style={{ transform: showExamReminders ? 'translateX(20px)' : 'translateX(0)' }} 
                    />
                  </button>
                </div>
                <div className="listItem">
                  <div>
                    <div style={{ fontWeight: 600, marginBottom: 4 }}>AI Voice Responses</div>
                    <div className="muted" style={{ fontSize: 12 }}>Enable voice for Dash AI responses</div>
                  </div>
                  <button 
                    onClick={() => setAiVoiceEnabled(!aiVoiceEnabled)}
                    className={`toggle ${aiVoiceEnabled ? 'toggleActive' : ''}`}
                  >
                    <span 
                      className="toggleThumb" 
                      style={{ transform: aiVoiceEnabled ? 'translateX(20px)' : 'translateX(0)' }} 
                    />
                  </button>
                </div>
                <div>
                  <label className="label">Data Usage Mode</label>
                  <select 
                    className="input"
                    value={dataUsageMode}
                    onChange={(e) => setDataUsageMode(e.target.value)}
                    style={{ marginTop: 'var(--space-2)' }}
                  >
                    <option value="low">Low (Saves data, fewer images)</option>
                    <option value="standard">Standard (Balanced)</option>
                    <option value="high">High (Best quality, uses more data)</option>
                  </select>
                </div>
              </div>
            </div>
