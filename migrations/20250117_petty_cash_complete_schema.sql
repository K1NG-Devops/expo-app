...
CREATE INDEX IF NOT EXISTS idx_petty_cash_accounts_active ON petty_cash_accounts(school_id, is_active) WHERE is_active = true;
...
CREATE INDEX IF NOT EXISTS idx_petty_cash_transactions_occurred_at ON petty_cash_transactions(school_id, occurred_at DESC);
...
CREATE POLICY "petty_cash_transactions_insert_policy" ON petty_cash_transactions
  ...
  WITH CHECK (
    is_super_admin() OR 
    (user_belongs_to_school(school_id) AND created_by = auth.uid())
  );
